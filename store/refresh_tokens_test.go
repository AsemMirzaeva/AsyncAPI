...
func TestRefreshTokenStore(t *testing.T) {
	...
	expetedExpiration, err := tokenPair.RefreshToken.Claims.GetExpirationTime()
	require.NoError(t, err)
	require.Equal(t, expetedExpiration.Time.UnixMilli(), refreshTokenRecord.ExpiresAt.UnixMilli())
	...
	require.Equal(t, refreshTokenRecord.CreateAt, refreshTokenRecord2.CreateAt)
